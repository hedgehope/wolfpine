...
**Audience:** anyone weighing a codegen or scheduling decision against Wolfpine's
...
documentation gives a range, Wolfpine charges the minimum. So the model
**under-predicts**, monotonically, and never over-predicts — which makes a
Wolfpine cycle count a *lower bound* on hardware, not an estimate of it.
...
**Wolfpine charges 6 cycles. Silicon delivers up to ~33.**
...
**What this means for codegen.** Any decision weighed against Wolfpine's cost
...
Wolfpine will under-state the cost of keeping them.
...
magnitude-dependent term is therefore unlicensable at every rank of Wolfpine's
...
Wolfpine's per-mechanism cycle attribution (rung 4) has three legs. Only one is
...
**A wrong-bank page lands wrong in Wolfpine too, and you will see it.** tt-metal
...
Wolfpine reproduces that exactly. It holds each bank as separate storage at its
...
Wormhole Wolfpine returns `errors=3072 of 6144` — no crash, no `TT_FATAL`, clean
...
fall through to a 0-bit shift and every page lands in bank 0". Wolfpine is not on
...
a 5.81 measured ceiling" — Wolfpine has nothing to say about it, and will not
...
is no PCIe tile (`framework/bridge/cores.py` stubs it to zeros) and no host-DMA
term anywhere in `framework/perf/`. A Wolfpine cycle count covers device-side work
only, and a host transfer costs zero of them. Do not read a Wolfpine run as
...
(`DramChannels` in `framework/device/tiles.py`, at the rate
...
That is the axis `perfbench/dramratebench` and `framework/perf/dram_rate_sweep.py`
already exercise.
...
that is too strong. Wolfpine catches the init-ordering faults that change the
...
Measured live against Wolfpine over the wire bridge (tt-metal 0.74, Wormhole,
...
(macro-op) expander configuration. Wolfpine models all three. So a reordering that
leaves a *different* value in one of them at the moment an op reads it changes
Wolfpine's answer, and a reordering that leaves the *same* values does not.
...
guaranteed last-writer-wins, Wolfpine reproduces it, and it is the mechanism behind
...
`framework/pe/tensix/mop_clobber_test.py`, in both directions — a clobber changes
...
and Wolfpine's configuration writes land instantly. It has no schedule in which the
...
op — and Wolfpine observes only the Tensix instruction stream. In the missed rows
...
**It runs against Wolfpine — but only from 2026-08-20.** The sanitizer reports
through DPRINT, and DPRINT could not start on Wolfpine before that date: every
run with it enabled aborted in `TT_THROW: Timed out writing init magic`
(`dprint_server.cpp:147`) because Wolfpine's stand-in cores zero-filled host
reads, so the host could never read back the magic word it had just written.
If you tried the sanitizer against Wolfpine and it would not start, that was
...
`pack_untilize_dest` needs is modelled (`framework/pe/tensix/backends/packer.py`,
pinned by `framework/pe/tensix/pack_strided_test.py`).
...
never lands before the pack that needs it. Traced in Wolfpine: in the early form
...
| Wolfpine | `NotImplementedError: PACR DST_ACCESS_STRIDED_MODE with DEST_ACCESS_CFG remap_addrs=0 swizzle_32b=0 …` |
...
neither specified nor referenced anywhere, and Wolfpine refuses rather than invent
...
a genuine Wolfpine defect and was fixed (`optests/packuntilizeinit`, the Wait Gate
...
transfer/barrier per item — is the standard dataflow optimisation. Wolfpine
...
that loop dominates — Wolfpine's marginal per-transaction cost has been checked
...
Wolfpine is **systematically optimistic about batching**, and the error runs the
same direction every time.

**This is measured against silicon, not theorised.** The nekbone team ported
six variants and ran them on both (2026-08-21). Wolfpine predicts the batched
...
`framework/network/noc_cost_model_test.py::test_an_extra_batched_read_costs_bandwidth_and_nothing_else`,
...
out. That is a statement about coverage, not about hardware: nothing in Wolfpine
