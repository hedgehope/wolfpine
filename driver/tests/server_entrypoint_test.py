...
``framework/bridge/device.py`` and imported by both servers from the package
façade, but ``framework/bridge/__init__.py`` was not updated to re-export it. The
...
def test_the_bridge_facade_exports_everything_it_advertises():
    ...
    The servers import from ``framework.bridge`` rather than
    ``framework.bridge.device``, so a name that reaches ``__all__`` without
    ...
    """
    import framework.bridge as bridge

    missing = [name for name in bridge.__all__ if not hasattr(bridge, name)]
    assert not missing, (
        f"framework.bridge.__all__ advertises {missing}, which cannot be imported "
        "from it. Add them to the `from framework.bridge.<module> import (...)` "
