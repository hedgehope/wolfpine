...
| Wolfpine's reconstruction of the issue loop | 18 | 19 |
...
Wolfpine as **+4 cycles on Wormhole and +1 on Blackhole** — and NRB4's source
...
transactions issued back to back before one barrier. Wolfpine charges the marginal
...
reads and writes; Wolfpine predicts it **wins** its first pass by 1.05–1.12×, and
silicon has it **losing** by 7–9 %. Every term Wolfpine charges is a real published
...
Generated by running **this program** against Wolfpine with the cost model on
...
**Below the crossover, Wolfpine says the two arms are the same number.** At 512 B
...
agrees with Wolfpine" is exactly what the broken run prints.
...
when it wants one. Wolfpine models the same sticky behaviour
(`extract_bits(self.packet_tag, 4, 10)` in `framework/network/tt_noc.py`). The
...
`framework/perf/unit_costs.yaml` only as a `corroboration` field on an existing
entry, never as provenance, and it does not by itself become a charged cost
term. The rules are in `framework/perf/costs.py`: every bound is charged at its low
...
### What Wolfpine says about the two arms, and why it is not an answer

Differencing the `burst` axis of those four files:

| | stateless | stateful | the loop bought |
| --- | --- | --- | --- |
| Wolfpine, Wormhole, **cost model OFF** | 38.00 cycles/tx | 23.00 | **15.00** |
| Wolfpine, Blackhole, **cost model OFF** | 41.00 | 23.00 | **18.00** |
| Wolfpine, Wormhole, **cost model ON** | **44.00** | **29.00** | **15.00** |
| Wolfpine, Blackhole, **cost model ON** | **47.00** | **29.00** | **18.00** |
...
`framework/perf/noc_issue_loop_test.py:180`. Comparing one to a card is a category
...
per-transaction term. It is not one. With the model on, Wolfpine reads 44.00 and
...
- **Wolfpine's stateless arm is unchanged by this work.** 38.00 and 41.00 are what
  the NRB2 program cost here, bit-for-bit, at all three burst intervals. That is
  the control check the card cannot do at home.
- **Wolfpine's saving does not match either of the dataset's**, and on Blackhole it
  goes the wrong way: the dataset says the shorter loop buys 1.0 cycles there and
  7.67 on Wormhole, while Wolfpine says 18 and 15. Nor is it the ~1–2 cycles that
  `framework.perf.noc_issue_loop`'s reconstruction would predict from removing one
...
- **That mismatch is structural, not a bug to tune out.** Wolfpine's NIU appends to
  ...
  is the signature of something Wolfpine does not model, and the simulator cannot
  be the witness for or against it. **Nothing in `framework/perf/` was changed to
  narrow the gap.**
...
`CMD_BUF_AVAIL` and `CMD_BUF_OVFL`, and Wolfpine's NUI raised
...
for "absent". See `framework/network/noc_registers_test.py`.

The simulator answer is a **known null and is not a result**: Wolfpine's NIU
