...
#                     payloads out of a worker's L1. Wolfpine's ABSOLUTE
...
"what does ONE MORE DRAM read in the same barrier cost". Wolfpine charges that
...
its DRAM reads and writes; Wolfpine predicts it WINS its first pass by 1.05-1.12x
...
echo "-----------------------------------------------"
echo "Generated by running THIS PROGRAM against Wolfpine with the cost model on and"
...
batched 2 KB DRAM read lands MEANINGFULLY ABOVE Wolfpine's 86.
...
if [ "$DRAM" -eq 1 ]; then
  ...
  echo ""
  echo "   Wolfpine now has an instruction-level model of both arms that was"
