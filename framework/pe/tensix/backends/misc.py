...
class MiscellaneousUnit(TensixBackendUnit):
    ...
    def handle_setdvalid(self, instruction_info, issue_thread, instr_args):
        ...
        1. REAL BLACKHOLE KERNELS ISSUE IT. ``TTI_SETDVALID(0b10)`` appears in
           ...
           datacopy) and in ``ckernel_debug.h``. The four opcodes Wolfpine does
           reject on Blackhole are ones no kernel reaches; this is not one of
           them, and refusing it would break a compute path Wolfpine exists to
        ...
        2. THE ILL-SPECIFIED PART IS ALREADY MODELLED AS WELL AS IT CAN BE.
           ...
           stale/held copy of a previous unpack's output format". Wolfpine's
           ...
           the last unpack into it, and nothing here disturbs it. So Wolfpine
        ...
        3. NO DIFFERENTIAL IS AVAILABLE EITHER WAY. Because the vendor sim
           ...
           check Wolfpine against it there. Refusing would not buy a check; it
        ...
        - The ``UnpredictableValue`` itself. A kernel that depends on the
          ...
          behaviour the docs tell it not to rely on; Wolfpine will give it the
        ...
        - The instruction's own precondition. Handing a bank to the Matrix Unit
          ...
          and Wolfpine does it without complaint. That is exactly what
          ...
          docs/plans/matrix-unit-thread-contention.md. Wolfpine has no per-bank
